...
<?php //$timeslot_theme = str_replace(array('&',',',' ', '/'), array('','','-', ''),strtolower($node->node_data_field_om_timeslot_theme_field_om_timeslot_theme_nid)); ?>	 
  <div class="calendar monthview day-event term-<?php print $node->term_data_tid; ?> theme-<?php print $timeslot_theme ?>" id="<?php print $node->date_id ?>">
    ...
    <?php foreach ($fields as $field): ?>
      <div id="<?php print $field['id']; ?>" class="view-field view-data-<?php print $field['id'] ?>">
        ...
        <?php endif; ?>  
        <?php if($field['id'] == 'nid'){ ?>
          <?php print om_timeslot_scheduler_format_calendar_display($field['data'], strtotime($calendar_start), strtotime($calendar_end), $_GET['tid']); ?>
        <?php } else { ?>
